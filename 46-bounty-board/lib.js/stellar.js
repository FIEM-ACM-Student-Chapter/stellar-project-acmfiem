...
export const createBounty = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.creator) throw new Error("creator address is required");

    return invokeWrite("create_bounty", [
        toSymbol(payload.id),
        new Address(payload.creator).toScVal(),
        nativeToScVal(payload.title || ""),
        nativeToScVal(payload.description || ""),
        toI128(payload.reward),
        toU64(payload.createdAt),
    ]);
};

export const assignBounty = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.creator) throw new Error("creator address is required");
    if (!payload?.worker) throw new Error("worker address is required");

    return invokeWrite("assign_bounty", [
        toSymbol(payload.id),
        new Address(payload.creator).toScVal(),
        new Address(payload.worker).toScVal(),
    ]);
};

export const submitBounty = async (id, worker) => {
    if (!id) throw new Error("id is required");
    if (!worker) throw new Error("worker address is required");

    return invokeWrite("submit_bounty", [
        toSymbol(id),
        new Address(worker).toScVal(),
    ]);
};

export const approveBounty = async (id, creator) => {
    if (!id) throw new Error("id is required");
    if (!creator) throw new Error("creator address is required");

    return invokeWrite("approve_bounty", [
        toSymbol(id),
        new Address(creator).toScVal(),
    ]);
};

export const closeBounty = async (id, creator) => {
    if (!id) throw new Error("id is required");
    if (!creator) throw new Error("creator address is required");

    return invokeWrite("close_bounty", [
        toSymbol(id),
        new Address(creator).toScVal(),
    ]);
};
...
export const getBounty = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_bounty", [toSymbol(id)]);
};

export const listBounties = async () => {
    return invokeRead("list_bounties", []);
};
