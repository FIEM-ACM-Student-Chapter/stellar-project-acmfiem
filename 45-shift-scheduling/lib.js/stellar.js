...
export const createShift = async (payload) => {
    ...
    if (!payload?.manager) throw new Error("manager address is required");

    return invokeWrite("create_shift", [
        toSymbol(payload.id),
        new Address(payload.manager).toScVal(),
...
export const assignShift = async (payload) => {
    ...
    return invokeWrite("assign_shift", [
        toSymbol(payload.id),
        new Address(payload.manager).toScVal(),
...
export const startShift = async (id, worker) => {
    if (!id) throw new Error("id is required");
    if (!worker) throw new Error("worker address is required");

    return invokeWrite("start_shift", [
        toSymbol(id),
        new Address(worker).toScVal(),
    ]);
};

export const completeShift = async (id, worker) => {
    if (!id) throw new Error("id is required");
    if (!worker) throw new Error("worker address is required");

    return invokeWrite("complete_shift", [
        toSymbol(id),
        new Address(worker).toScVal(),
    ]);
};
...
export const getShift = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_shift", [toSymbol(id)]);
};

export const listShifts = async () => {
    return invokeRead("list_shifts", []);
};

export const getActiveShiftCount = async () => {
    return invokeRead("get_active_shift_count", []);
};
