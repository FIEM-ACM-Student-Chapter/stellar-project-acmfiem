...
export const issuePass = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.host) throw new Error("host address is required");
    if (!payload?.visitor) throw new Error("visitor address is required");

    return invokeWrite("issue_pass", [
        toSymbol(payload.id),
        new Address(payload.host).toScVal(),
        new Address(payload.visitor).toScVal(),
        nativeToScVal(payload.purpose || ""),
        nativeToScVal(payload.location || ""),
        toU64(payload.visitTime),
    ]);
};

export const approvePass = async (id, host) => {
    if (!id) throw new Error("id is required");
    if (!host) throw new Error("host address is required");

    return invokeWrite("approve_pass", [
        toSymbol(id),
        new Address(host).toScVal(),
    ]);
};

export const checkInVisitor = async (id, visitor) => {
    if (!id) throw new Error("id is required");
    if (!visitor) throw new Error("visitor address is required");

    return invokeWrite("check_in_visitor", [
        toSymbol(id),
        new Address(visitor).toScVal(),
    ]);
};

export const checkOutVisitor = async (id, visitor) => {
    if (!id) throw new Error("id is required");
    if (!visitor) throw new Error("visitor address is required");

    return invokeWrite("check_out_visitor", [
        toSymbol(id),
        new Address(visitor).toScVal(),
    ]);
};

export const revokePass = async (id, host) => {
    if (!id) throw new Error("id is required");
    if (!host) throw new Error("host address is required");

    return invokeWrite("revoke_pass", [
        toSymbol(id),
        new Address(host).toScVal(),
    ]);
};
...
export const getPass = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_pass", [toSymbol(id)]);
};

export const listPasses = async () => {
    return invokeRead("list_passes", []);
};
