...
export const createSession = async (payload) => {
    ...
    return invokeWrite("create_session", [
        ...
        toU64(payload.sessionTime),
    ]);
};

export const checkIn = async (id, attendee) => {
    if (!id) throw new Error("id is required");
    if (!attendee) throw new Error("attendee address is required");

    return invokeWrite("check_in", [
        toSymbol(id),
        new Address(attendee).toScVal(),
    ]);
};

export const closeSession = async (id, organizer) => {
    if (!id) throw new Error("id is required");
    if (!organizer) throw new Error("organizer address is required");

    return invokeWrite("close_session", [
        toSymbol(id),
...
export const getAttendeeCount = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_attendee_count", [toSymbol(id)]);
};
