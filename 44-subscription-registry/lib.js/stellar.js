...
export const createSubscription = async (payload) => {
    ...
    if (!payload?.subscriber) throw new Error("subscriber address is required");

    return invokeWrite("create_subscription", [
        toSymbol(payload.id),
        new Address(payload.subscriber).toScVal(),
        nativeToScVal(payload.planName || ""),
        toSymbol(payload.billingCycle || "monthly"),
        toI128(payload.fee),
...
export const renewSubscription = async (payload) => {
    ...
    return invokeWrite("renew_subscription", [
        toSymbol(payload.id),
        new Address(payload.subscriber).toScVal(),
...
export const pauseSubscription = async (id, subscriber) => {
    if (!id) throw new Error("id is required");
    if (!subscriber) throw new Error("subscriber address is required");

    return invokeWrite("pause_subscription", [
        toSymbol(id),
        new Address(subscriber).toScVal(),
    ]);
};

export const cancelSubscription = async (id, subscriber) => {
    if (!id) throw new Error("id is required");
    if (!subscriber) throw new Error("subscriber address is required");

    return invokeWrite("cancel_subscription", [
        toSymbol(id),
        new Address(subscriber).toScVal(),
    ]);
};
...
export const getSubscription = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_subscription", [toSymbol(id)]);
};

export const listSubscriptions = async () => {
    return invokeRead("list_subscriptions", []);
};
