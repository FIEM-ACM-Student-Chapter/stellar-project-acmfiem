...
export const createMember = async (payload) => {
    ...
    return invokeWrite("create_member", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        nativeToScVal(payload.name || ""),
        toSymbol(payload.tier || "bronze"),
...
export const redeemPoints = async (payload) => {
    ...
    return invokeWrite("redeem_points", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        toI128(payload.amount),
    ]);
...
export const updateTier = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.owner) throw new Error("owner address is required");

    return invokeWrite("update_tier", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        toSymbol(payload.tier || "bronze"),
    ]);
