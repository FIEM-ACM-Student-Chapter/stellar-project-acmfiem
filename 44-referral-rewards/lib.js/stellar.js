...
export const createCampaign = async (payload) => {
    ...
    if (!payload?.promoter) throw new Error("promoter address is required");

    return invokeWrite("create_campaign", [
        toSymbol(payload.id),
        new Address(payload.promoter).toScVal(),
        nativeToScVal(payload.title || ""),
        toI128(payload.rewardAmount),
        toU64(payload.createdAt),
    ]);
};

export const registerReferral = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.referrer) throw new Error("referrer address is required");
    if (!payload?.friend) throw new Error("friend address is required");

    return invokeWrite("register_referral", [
        toSymbol(payload.id),
        new Address(payload.referrer).toScVal(),
        new Address(payload.friend).toScVal(),
    ]);
};

export const approveReferral = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.promoter) throw new Error("promoter address is required");
    if (!payload?.friend) throw new Error("friend address is required");

    return invokeWrite("approve_referral", [
        toSymbol(payload.id),
        new Address(payload.promoter).toScVal(),
        new Address(payload.friend).toScVal(),
    ]);
...
export const issueReward = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.promoter) throw new Error("promoter address is required");
    if (!payload?.friend) throw new Error("friend address is required");

    return invokeWrite("issue_reward", [
        toSymbol(payload.id),
        new Address(payload.promoter).toScVal(),
        new Address(payload.friend).toScVal(),
    ]);
...
export const getReferral = async (id) => {
    ...
    return invokeRead("get_referral", [toSymbol(id)]);
};

export const listReferrals = async () => {
    return invokeRead("list_referrals", []);
};

export const getRewardCount = async () => {
    return invokeRead("get_reward_count", []);
};
