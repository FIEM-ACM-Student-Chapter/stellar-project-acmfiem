...
export const registerWarranty = async (payload) => {
    ...
    return invokeWrite("register_warranty", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        new Address(payload.provider).toScVal(),
        nativeToScVal(payload.productName || ""),
        nativeToScVal(payload.serialNumber || ""),
        toU64(payload.purchasedAt),
        toU64(payload.expiresAt),
    ]);
};

export const transferWarranty = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.owner) throw new Error("owner address is required");
    if (!payload?.newOwner) throw new Error("new owner address is required");

    return invokeWrite("transfer_warranty", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        new Address(payload.newOwner).toScVal(),
    ]);
};

export const fileClaim = async (payload) => {
    if (!payload?.id) throw new Error("id is required");
    if (!payload?.owner) throw new Error("owner address is required");

    return invokeWrite("file_claim", [
        toSymbol(payload.id),
        new Address(payload.owner).toScVal(),
        nativeToScVal(payload.claimNote || ""),
    ]);
};

export const closeClaim = async (id, provider) => {
    if (!id) throw new Error("id is required");
    if (!provider) throw new Error("provider address is required");

    return invokeWrite("close_claim", [
        toSymbol(id),
...
export const getWarranty = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_warranty", [toSymbol(id)]);
};

export const listWarranties = async () => {
    return invokeRead("list_warranties", []);
};

export const getWarrantyCount = async () => {
    return invokeRead("get_warranty_count", []);
};
