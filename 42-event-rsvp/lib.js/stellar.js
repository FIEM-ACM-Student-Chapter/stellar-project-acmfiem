...
export const createEvent = async (payload) => {
    ...
    return invokeWrite("create_event", [
        toSymbol(payload.id),
        new Address(payload.host).toScVal(),
        nativeToScVal(payload.title || ""),
        nativeToScVal(payload.venue || ""),
        toU32(payload.capacity),
        toU64(payload.eventTime),
    ]);
};

export const rsvpEvent = async (id, attendee) => {
    if (!id) throw new Error("id is required");
    if (!attendee) throw new Error("attendee address is required");

    return invokeWrite("rsvp_event", [
        toSymbol(id),
        new Address(attendee).toScVal(),
    ]);
};

export const confirmAttendance = async (id, host, attendee) => {
    if (!id) throw new Error("id is required");
    if (!host) throw new Error("host address is required");
    if (!attendee) throw new Error("attendee address is required");

    return invokeWrite("confirm_attendance", [
        toSymbol(id),
...
export const closeEvent = async (id, host) => {
    if (!id) throw new Error("id is required");
    if (!host) throw new Error("host address is required");

    return invokeWrite("close_event", [
        toSymbol(id),
        new Address(host).toScVal(),
    ]);
};
...
export const getEvent = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_event", [toSymbol(id)]);
};

export const listEvents = async () => {
    return invokeRead("list_events", []);
};

export const getRsvpCount = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_rsvp_count", [toSymbol(id)]);
};
