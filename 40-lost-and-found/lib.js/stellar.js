...
export const reportItem = async (payload) => {
    ...
    return invokeWrite("report_item", [
        ...
        toU64(payload.reportedAt),
    ]);
};

export const claimItem = async (id, claimant) => {
...
export const markResolved = async (id, reporter) => {
    if (!id) throw new Error("id is required");
    if (!reporter) throw new Error("reporter address is required");

    return invokeWrite("mark_resolved", [
        toSymbol(id),
...
export const getItem = async (id) => {
    if (!id) throw new Error("id is required");
    return invokeRead("get_item", [toSymbol(id)]);
};

export const listItems = async () => {
    return invokeRead("list_items", []);
};
